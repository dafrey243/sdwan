...
#            exit()
        ...
        apilist = ["/smartLicensing/removeSaVaSelection"]
        for api_url in apilist:
            url = base_url + api_url
            print(url)
            payload = {"smartAccountsToBeDeleted":[]}
            response = requests.post(
                url=url, headers=headers, data=json.dumps(payload), verify=False)
            if response.status_code == 200:
                mydata = json.loads(response.text)
        ...
        apilist = ["/smartLicensing/syncLicenses"] 
        for api_url in apilist:
            url = base_url + api_url
            print(url)
            payload = {"mode":"online","licenseType":"prepaid","multipleEntitlement":False,"smartAccounts":[{"account_id":myaccountid,"virtual_accounts":[{"virtual_account_id":myvaaccountid}]}]}

            files = {
                'file': ('license.txt', open('license.txt', 'rb')),
                'data': (None, json.dumps(payload), 'application/json')
            }

            headers = {'Accept': 'application/json', 'Cookie': jsessionid, 'X-XSRF-TOKEN': token}

            response = requests.post(
                #url=url, headers=headers, data=payload, verify=False)
                url=url, headers=headers, files=files, verify=False)
            if response.status_code == 200:
                #mydata = json.loads(response.text)
                print(json.dumps(mydata, indent=4))
                print(response.text)
