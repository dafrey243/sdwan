...
if __name__ == '__main__':
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            x = PrettyTable()
            x.field_names = ['seq', 'Pref Color', 'backupColor',
                             'match', 'match value', 'tosdec', 'toshex', 'sla', 'latency', 'loss', 'jitter']
            ...
            for item in sequencelist:
                ...
                if (sequenceId <= 1310):
                    ...
                    matchfield = matchdict['entries'][0]['field']
                    entry = len(matchdict['entries'])
                    loop = 0
                    while loop < entry:
                        matchfield = matchdict['entries'][loop]['field']
                        if (matchfield == "dscp"):
                            dscpvalue = int(
                                matchdict['entries'][loop]['value'])
                            tosdec = dscpvalue << 2
                            toshex = "{:#04x}".format(tosdec)

                        if (matchfield == "appList"):
                            appListRef = matchdict['entries'][loop]['ref']
                            tosdec = "N/A"
                            toshex = "N/A"

                           # New
                            api_url = "/template/policy/list/app/" + appListRef
                            url = base_url + api_url
                            response = requests.get(
                                url=url, headers=headers, verify=False)
                            if response.status_code == 200:
                                myapplist = json.loads(response.text)
                                #print(json.dumps(myapplist, indent=4))
                                dscpvalue = myapplist.get('name')
                        loop += 1
                        ...
                        if len(pmeter) == 3:
                            ...
                            ptype = "None"
                            x.add_row([sequenceId, value, backupColor, matchfield,
