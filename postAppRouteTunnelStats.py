            ...
            if response.status_code == 200:
                ...
                for item in mydict["data"][0:1]:
                    for k, v in item.items():
                        ...
                        if re.search("^(?!vdevice-dataKey|host|src|dst|app|local_color|remote|color|remote_system_ip|sla|id|fec|entry_time|statcycletime|vmanage_system_ip|vdevice_name|device_model|vip_idx|tenant|tx_octets|rx_octets|window|proto)", k):
                            headerrow.append(k)
                ...
                for item in mydict["data"]:
                    ...
                    for item1 in headerrow:
                        var = item.get(item1)
                        if item1 == 'summary_time':
                            lastupdated = item.get('summary_time')
