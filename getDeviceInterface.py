...
if __name__ == '__main__':
    ...
    try:
        ...
        # GET
        apilist = ["/device/interface?deviceId=" + DEVICE_ID + '&&',
                   "/device/interface/queue_stats?deviceId=" + DEVICE_ID + '&&']
             ...
             #   print(json.dumps(mydata, indent=4))
                ...
                for item in mydict["data"][0:1]:
                    for k, v in item.items():
                        ...
                        if re.search("^(?!interface|auto|flaps|type|bia|ipv6|octet|tx|rx|vdevice|secondary|allow-service|uptime|port|hwaddr).", k):
                            headerrow.append(k)
                            # Replace dash with newline to make multiline cell headerrow
                            k = k.replace("-", "\n")
                            # Add newline at each capital letter in camelHump syntax for multiline cell
                            k = re.sub("([A-Z])", '\n' r'\1', k)
                            pheaderrow.append(k)
