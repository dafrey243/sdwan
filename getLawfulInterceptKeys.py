...
if __name__ == '__main__':
    ...
    try:
        ...
        if len(sys.argv) == 2: 
           interceptid = sys.argv[1]
        else:
            print("Requires intercept id as argument\n")
            exit()

        # GET 
        apilist = ["/li/intercept/retrieve/" + interceptid]
        for api_url in apilist:
            ...
            if response.status_code == 200:
                ...
                for item in mydict["data"]["records"][0:1]:
                    for k, v in item.items():
                        # Take out keys when header row is too long
                        # Iterate over headerrow, and use pheaderrow for printing
                        if re.search("^(?!vdevice-dataKey|encap|private-ipv6-address|private-ipv6-port|public-ipv6-address|public-ipv6-port)", k):
                            headerrow.append(k)
                ...
                for item in mydict["data"]["records"]:
                    row = list()
