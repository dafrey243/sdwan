...
if __name__ == '__main__':
    ...
    try:
        ...
        # GET
	apilist = ["/device/cloudx/applications?deviceId=" + DEVICE_ID + '&&', 
		   "/device/cloudx/gatewayexits?deviceId=" + DEVICE_ID + '&&', 
		   "/device/cloudx/localexits?deviceId=" + DEVICE_ID + '&&']
	for api_url in apilist:
            ...
            if response.status_code == 200:
                ...
                table = list()
                pheaderrow = ['remote-color', 'latency', 'site-id', 'vqe-score', 'vqe-status', 'loss', 'exit-type', 'local-color', 'vdevice\nhost-name',
                        'gateway\nsystem-ip', 'gateway-ip', 'interface', 'application']
                headerrow = ['remote-color', 'latency', 'site-id', 'vqe-score', 'vqe-status', 'loss', 'exit-type', 'local-color', 'vdevice-host-name',
			'gateway-system-ip', 'gateway-ip', 'interface', 'application']
