        ...
        #   print(json.dumps(mydata, indent=4))
            ...
            for item in mydict["data"]:
                ...
                if DEVICE_ID is not None:
                    ...
                    apilist = [
                        "/device/control/connections?deviceId=" + DEVICE_ID]
                        ...
                        # Creates the header row.  camelHump and/or dash will be multiline cell
                            for item in mydict["data"][0:1]:
                                for k, v in item.items():
                                    ...
                                    if re.search("^(?!vdevice-dataKey|uptime-date|vdevice-name|instance|v-org-name|controller-group-id|domain-id)", k):
                                        headerrow.append(k)
                        ...
                        # Iterate of data to create row.  Boundry of a row defined by headerrow (keys)
                            for item in mydict["data"]:
                                ...
                                for item1 in headerrow:
                                    ...
                                    if item1 == 'lastupdated':
                                        ...
                                        var = datetime.datetime.fromtimestamp(
                                            lastupdated)
                                    if item1 == 'idle-timeout-date':
                                        lastupdated = item.get(
                                            'idle-timeout-date')
                                        lastupdated = lastupdated / 1000
                                        var = datetime.datetime.fromtimestamp(
                                            lastupdated)
                            ...
                            try:
                                ...
                                print(tabulate.tabulate(
                                    table, pheaderrow, tablefmt="pretty"))
