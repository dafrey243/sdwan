...
if __name__ == '__main__':
    ...
    try:
        ...
        if len(sys.argv) >= 3:
            ...
            print(
                "or 1)system-ip of the device and application values 11.11.1.2 https http\n")
        ...
        # Get Acutal network attributes from AppAware

        os.system('python3 getDeviceAppAwareStats.py %s %s' % (deviceIp, remoteSystemIp))
        ...
        for dscp in dscp:
           # print(type(dscp))
            ...
            # if not DSCP then we will match on App (string). dscp will be used to feed in app argument.
            else:
                payload = {
                    "remoteSystemIp": remoteSystemIp,
                    "vpn": vpn,
            ...
            if response.status_code == 200:
                ...
                x.field_names = ['device', 'localColor', 'destIp', 'remoteSystemIp',
                                 'sourcePort', 'sourceIp', 'destPort', 'remoteColor', 'match']
                for item in mydata['nexthops']:
                    ...
                    x.add_row([deviceIp, localColor, destIp, remoteSystemIp,
                              sourcePort, sourceIp, destPort, remoteColor, dscp])
