...
if __name__ == '__main__':
    ...
    try:
        ...
        if len(sys.argv) == 2:
            ...
            print("One argument required. 1) ProfileID in network design\n")
            os.system("python getNetworkDesign.py")
            exit()

        # GET
        apilist = ["/networkdesign/profile/status/" + DEVICE_ID ]
            ...
            if response.status_code == 200:
                ...
                for item in mydict["data"][0:1]:
                    for k, v in item.items():
                        ...
                        if re.search("^(?!vdevice-dataKey|platformFamily|deviceType|ncsDeviceName|templateApplyLog|managementSystemIP|chasisNumber|configStatusMessageDetails|deviceModel|vedgeCertificateState|rootCertHash|personality|uploadSource|subjectSerialNumber|local-system-ip|system-ip|model_sku|vbond|lastupdated|availableVersions|template|draftMode|lifeCycleRequired|expirationDate|hardwareCertSerialNumber)", k):
                            headerrow.append(k)
                ...
                for item in mydict["data"]:
                    ...
                    for item1 in headerrow:
                        ...
                        if item1 == 'uptime-date':
                            lastupdated = item.get('uptime-date')
                            lastupdated = lastupdated / 1000
                            var = datetime.datetime.fromtimestamp(lastupdated)
                        if var is None:
                            var = 'None'
