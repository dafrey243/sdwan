...
if __name__ == '__main__':
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            table = list()
            devicetable = list()
            pheaderrow = ['deviceId', 'host-name', 'site-id', 'state', 'control\nConnections', 'personality', 'device\nmodel',
                          'omp\nPeers', 'bfd\nSessions', 'reachability', 'platform', 'version']

            headerrow = ['deviceId', 'host-name', 'site-id', 'state', 'controlConnections', 'personality', 'device-model',
...
#                if state_description == "All daemon's are up and running. Day-0 config applied successfully.":
                    #state_description = state_description.replace(".", "\n")
                    #state_description = re.sub(".", '\n', state_description)
            ...
            try:
                ...
                continueon = raw_input(
                    'Run getDeviceMonitorDetail.py Enter y to continue: ')
                if continueon == "y":
                    os.system('python getDeviceMonitorDetail.py')
                else:
                    exit()
